...
function applyProfileBasedNavVisibility(user) {
  ...
  if (inspectionEl) {
    inspectionEl.style.display = userHasInspectionNavAccess(user)
      ? "list-item"
      : "none";
  }
  ...
  if (tktEl) {
    tktEl.style.display =
      user && (user.role === "admin" || user.access_ticketing === true)
        ? "list-item"
        : "none";
  }
  ...
  if (opsEl) {
    ...
    opsEl.style.display = opsNav ? "list-item" : "none";
  }
  const reportEl = document.getElementById("report-gen-menu-item");
  if (reportEl) {
    reportEl.style.display = userHasReportGenerationNavAccess(user)
      ? "list-item"
      : "none";
  }
}
...
function checkAndShowAdminMenu(user) {
  ...
  const deviceMgmtMenuItem = document.getElementById(
  ...
  const submittedFormsMenuItem = document.getElementById(
    "submitted-forms-menu-item",
  );
  ...
  if (submittedFormsMenuItem) {
    ...
    submittedFormsMenuItem.classList.toggle(
      "has-submitted-dropdown",
      !!showSubmitted,
    );
  ...
  if (adminMenuItem) {
    adminMenuItem.style.display =
      user && user.role === "admin" ? "list-item" : "none";
  }
  if (deviceMgmtMenuItem) {
    deviceMgmtMenuItem.style.display =
      user && user.role === "admin" ? "list-item" : "none";
  }
  if (bdModuleMenuItem) {
  ...
  if (financeMenuItem) {
    const showFinance =
      user &&
      (user.role === "admin" ||
        user.access_finance === true ||
        (user.designation &&
          ["general_manager", "operations_manager"].includes(
            user.designation,
          )));
...
function playDashboardModuleEntrance() {
  ...
  if (window.innerWidth <= 768) {
    ...
    Array.from(grid.querySelectorAll(":scope > .module-card")).forEach(
      function (card) {
...
function updateModuleVisibility(user) {
  ...
  // Check Fire system inspection access
  ...
  const showSubmittedFormsMod = shouldShowModule(
    userHasSubmittedFormsModuleAccess(user),
  );
  if (submittedFormsCard) {
    submittedFormsCard.style.display = showSubmittedFormsMod ? "block" : "none";
    submittedFormsCard.style.visibility = showSubmittedFormsMod
      ? "visible"
      : "hidden";
  }
  if (submittedFormsMenuItem) {
    submittedFormsMenuItem.style.display = showSubmittedFormsMod
      ? "list-item"
      : "none";
  }
  ...
  const storeCard = document.getElementById("module-store");
  const storeMenuItem = document.getElementById("store-menu-item");
  const hasStoreAccess = shouldShowModule(
    user.access_procurement_module === true,
  );
  if (storeCard) {
    storeCard.style.display = hasStoreAccess ? "block" : "none";
    storeCard.style.visibility = hasStoreAccess ? "visible" : "hidden";
  }
  if (storeMenuItem) {
    storeMenuItem.style.display = hasStoreAccess ? "list-item" : "none";
  }
  ...
  // Finance & Invoicing — dedicated flag (GM/OM keep access for convenience)
  const financeCard = document.getElementById("module-finance");
  if (financeCard) {
    const showFinance = shouldShowModule(
      user.access_finance === true ||
        (user.designation &&
          ["general_manager", "operations_manager"].includes(user.designation)),
    );
    financeCard.style.display = showFinance ? "block" : "none";
    financeCard.style.visibility = showFinance ? "visible" : "hidden";
  }

  // DocHub — match navbar DocHub visibility
  const dochubCard = document.getElementById("module-dochub");
  if (dochubCard) {
    const showDochub = userHasDocHubNavAccess(user);
    dochubCard.style.display = showDochub ? "block" : "none";
    dochubCard.style.visibility = showDochub ? "visible" : "hidden";
  }

  // Administrative — admin only (same as navbar Administrative item)
  ...
  if (modulesSection) {
    const visibleCount = modulesGrid
      ? Array.from(modulesGrid.children).filter(function (card) {
          ...
          return (
            card.classList.contains("module-card") &&
...
async function loadPendingCount(user) {
  ...
  const isReviewer =
    user &&
    ((user.designation && reviewerDesignations.includes(user.designation)) ||
      user.access_business_development === true);
  const isSupervisor = user && user.designation === "supervisor";
  const pendingReviewMenuItem = document.getElementById(
    "pending-review-menu-item",
  );
...
function loadProfileData() {
  ...
  if (!token) {
    ...
    profileContent.innerHTML =
      '<div style="text-align: center; padding: 2rem;"><p style="color: var(--text-light);">Please log in to view your profile.</p></div>';
    return;
  }
...
function displayProfileData(user) {
  ...
  const getModuleAccess = () => {
    ...
    if (user.role === "admin" || user.access_procurement_module)
      modules.push("Store");
    if (
      user.role === "admin" ||
      user.designation === "business_development" ||
      user.access_business_development
    )
      modules.push("Sales");
    if (userHasReportGenerationNavAccess(user))
      modules.push("Report Generation");
...
function getProfileCardHTML(
  ...
  if (user.role === "admin" || user.access_procurement_module)
    modules.push({ name: "Store", color: "#7c3aed" });
  if (
    user.role === "admin" ||
    user.designation === "business_development" ||
    user.access_business_development
  )
    modules.push({ name: "Sales", color: "#0d9488" });
  if (userHasReportGenerationNavAccess(user))
    modules.push({ name: "Report Generation", color: "#0369a1" });
  ...
  const moduleBadges =
    modules.length > 0
      ? modules
          .map(
            (m) =>
              `<span class="pro-module-badge" style="--badge-color: ${m.color}">${m.name}</span>`,
          )
          .join("")
      : '<span class="pro-no-access">No modules assigned</span>';

  const hrJobTitle = escapeHtml(user.job_designation || "—");
  const annualLeavesDisp =
    user.annual_leave_days != null
      ? escapeHtml(String(user.annual_leave_days))
      : "—";
  const otherLeavesDisp =
    user.other_leave_days != null
      ? escapeHtml(String(user.other_leave_days))
      : "—";
 ...
 .pro-container textarea,
 .pro-container button,
 .pro-container label,
 .pro-container span,
 .pro-container p,
 .pro-container .pro-info-label,
 .pro-container .pro-info-value,
 .pro-container .pro-module-badge,
 ...
 .pro-profile-edit input[type="text"],
 .pro-profile-edit input[type="date"] {
 width: 100%;
 box-sizing: border-box;
 ...
 <div class="pro-user-id" title="Your account ID"><span>User ID</span> ${escapeHtml(String(user.id))}</div>
 </div>
 </div>
 </div>
 </div>
 ...
 <div class="pro-info-label">User ID</div>
 <div class="pro-info-value">${escapeHtml(String(user.id))}</div>
 </div>
 </div>
 <div class="pro-info-item">
 <div class="pro-info-content">
 ...
 <div class="pro-profile-edit" style="margin: 0.85rem 0 0; padding: 1rem 1rem; background: #f8fafc; border: 1px solid #e2e8f0;">
 <div style="font-size: 0.82rem; font-weight: 700; color: #334155; margin-bottom: 0.5rem;">Your details</div>
 <p style="font-size: 0.72rem; color: #64748b; margin: 0 0 0.55rem;">Update your login username and name used on documents. Changing your username sends a confirmation email. Job title, leave balances, and reporting manager are maintained by an administrator.</p>
 <div class="pro-profile-edit-grid">
 <label class="pro-mini-lbl"><span style="display:block;margin-bottom:.25rem;font-size:.74rem;color:#64748b;">Username</span>
 <input type="text" id="profileManagedUsername" name="amaan_login_username" value="${escapeHtml(user.username || "")}" maxlength="80" autocomplete="off" autocapitalize="none" autocorrect="off" spellcheck="false" /></label>
 <label class="pro-mini-lbl"><span style="display:block;margin-bottom:.25rem;font-size:.74rem;color:#64748b;">Full name</span>
 <input type="text" id="profileManagedFullName" value="${escapeHtml(user.full_name || "")}" maxlength="120" autocomplete="name" /></label>
 <label class="pro-mini-lbl"><span style="display:block;margin-bottom:.25rem;font-size:.74rem;color:#64748b;">Joined company</span>
 <input type="date" id="profileManagedJoined" value="${user.employment_start_date ? escapeHtml(String(user.employment_start_date).slice(0, 10)) : ""}" /></label>
 </div>
 ...
 <div class="pro-security-card ${user.password_changed && !user.password_expiry_warning ? "success" : "warning"}">
 <div class="pro-security-icon">${user.password_changed && !user.password_expiry_warning ? "✓" : "!"}</div>
 <div class="pro-security-content">
 <div class="pro-security-title">${
   user.password_expiry_warning
     ? "Password expiring soon"
     : user.password_changed
       ? "Password is secure"
       : "Password change required"
 }</div>
 <div class="pro-security-desc">${
   user.password_expiry_warning
     ? `Password expires in ${user.password_days_remaining != null ? user.password_days_remaining : "a few"} day(s). Update before the account locks.`
     : user.password_changed
       ? "Passwords must be updated every 65 days."
       : "Please update your temporary password for security"
 }</div>
 </div>
 <div class="pro-security-action">
 <button class="pro-btn ${user.password_changed && !user.password_expiry_warning ? "pro-btn-outline" : "pro-btn-primary"} pro-btn-sm" onclick="showChangePasswordForm()">
 ${user.password_changed && !user.password_expiry_warning ? "Change" : "Update Now"}
 </button>
 </div>
 </div>
  ...
  if (removeBtn) {
    removeBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const data = await response.json().catch(() => ({}));
        if (!response.ok)
          throw new Error(data.error || "Failed to remove defaults");
...
function initManagedProfileFields() {
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      const res = await fetch("/api/auth/profile", {
        ...
        body: JSON.stringify({
          ...
          employment_start_date:
            joinedEl && joinedEl.value ? joinedEl.value : "",
        }),
      });
      ...
      const savedUsername = data.user && data.user.username
        ? String(data.user.username).toLowerCase()
        : "";
      if (savedUsername && savedUsername !== wantedUsername) {
        throw new Error(
          "Username was not updated. Refresh the page and try again.",
        );
      }
      if (data.username_changed) {
        setHint(
          data.email_sent
            ? "Saved. Confirmation email sent for your new username."
      ...
      if (data.user) {
        ...
        if (usernameEl && data.user.username) {
          usernameEl.value = data.user.username;
        }
        const heroId = document.querySelector("#profileContent .pro-user-id");
        if (heroId && data.user.id != null) {
          heroId.innerHTML = `<span>User ID</span> ${escapeHtml(String(data.user.id))}`;
        }
      ...
      setHint(e.message || "Save failed.", "error");
    } finally {
      btn.disabled = false;
      setTimeout(() => {
        ...
        if (
          ...
          (h.textContent === "Saved." ||
            ...
            h.textContent.startsWith("Saved. "))
        ) {
          h.textContent = "";
...
function dashboardStatHrefFromLabel(label) {
  ...
  if (L.indexOf("material") >= 0 || L.indexOf("catalog") >= 0) return "/store/";
  if (
    L.indexOf("project") >= 0 ||
    L.indexOf("rfp") >= 0 ||
    L.indexOf("pipeline") >= 0
  )
    return "/admin/bd";
  if (L.indexOf("pending") >= 0) return "/workflow/pending-reviews";
  if (L.indexOf("completed") >= 0 || L.indexOf("completion rate") >= 0)
...
function applyDashboardStatCardLinks(metrics) {
  ...
  document
    .querySelectorAll(".dashboard-stat-card--clickable[data-metric-index]")
    .forEach(function (card) {
      var idx = parseInt(card.getAttribute("data-metric-index"), 10);
      var m = metrics[idx];
      var label = m && m.label ? m.label : "";
      var href =
        m && m.href ? String(m.href) : dashboardStatHrefFromLabel(label);
      ...
      if (Array.isArray(metrics) && metrics.length) {
        ...
        for (var i = 0; i < 4; i++) {
          ...
          if (lbl && m && m.label) {
            if (i === 3 && joinedRow) {
              ...
              if (joinedDate) {
                joinedRow.innerHTML =
                  escapeHtml(m.label) +
                  ' · Joined <a href="#" id="stat-joined-link-3" class="dashboard-stat-joined-link">' +
                ...
                joinedRow.innerHTML =
                  '<span id="stat-label-3">' + escapeHtml(m.label) + "</span>";
              }
            } else {
          ...
          if (i === 3) {
            ...
            var annual =
              m && m.annual_leave_days != null ? m.annual_leave_days : null;
            var other =
              m && m.other_leave_days != null ? m.other_leave_days : null;
        ...
        var formsEl =
          ...
          document.getElementById("stat-forms-submitted");
        var pendingEl =
          document.getElementById("stat-value-1") ||
          document.getElementById("stat-pending-review");
        var usersEl =
          document.getElementById("stat-value-2") ||
          document.getElementById("stat-active-users");
        var rateEl =
          document.getElementById("stat-value-3") ||
          document.getElementById("stat-completion-rate");
        if (document.getElementById("stat-label-0"))
          document.getElementById("stat-label-0").textContent =
            "Forms submitted";
        if (document.getElementById("stat-label-1"))
          document.getElementById("stat-label-1").textContent =
            "Pending review";
        if (document.getElementById("stat-label-2"))
          document.getElementById("stat-label-2").textContent = "Active users";
        if (document.getElementById("stat-label-3"))
          document.getElementById("stat-label-3").textContent =
            "Completion rate";
        if (formsEl)
          formsEl.textContent =
            typeof d.forms_submitted === "number"
              ? d.forms_submitted.toLocaleString()
              : d.forms_submitted != null
                ? d.forms_submitted
                : "0";
        if (pendingEl)
          pendingEl.textContent =
            typeof d.pending_review === "number"
              ? d.pending_review
              : d.pending_review != null
                ? d.pending_review
                : "0";
        if (usersEl)
          usersEl.textContent =
        ...
        if (rateEl)
          rateEl.textContent =
            typeof d.completion_rate === "number"
              ? d.completion_rate + "%"
        ...
        if (joinedRowFb) {
          ...
          if (lblFb)
            joinedRowFb.innerHTML =
              '<span id="stat-label-3">' +
              escapeHtml(lblFb.textContent || "Completion rate") +
              "</span>";
        }
...
function applyPasswordPolicyBanner(user, meta) {
  const el = document.getElementById("passwordPolicyBanner");
  if (!el || !user) return;
  const isAdmin =
    user.role === "admin" ||
    String(user.username || "").toLowerCase() === "admin";
  const reminder =
    !!(meta && meta.temp_password_reminder) || !!user.temp_password_reminder;
  const warn =
    !!(meta && meta.password_expiry_warning) || !!user.password_expiry_warning;
  const days =
    meta && meta.password_days_remaining != null
      ? meta.password_days_remaining
      : user.password_days_remaining;
  const needsChange =
    !user.password_changed || !!(meta && meta.requires_password_change);
  ...
  if (warn && days != null) {
    text =
      days <= 1
        ? "Your password expires tomorrow. Please update it now to avoid account lock."
        : `Your password expires in ${days} days. Please update it before day 66 when the account locks.`;
    kind = "warn";
  } else if (
    !isAdmin &&
    (reminder || (needsChange && !user.password_changed))
  ) {
    text =
      "Please change your temporary password. You can update it anytime from your profile.";
    kind = "remind";
  }
  ...
  el.hidden = false;
  el.className = "password-policy-banner " + kind;
  el.innerHTML =
    "<span>" +
    text +
    "</span>" +
    '<span class="ppb-actions"><button type="button" class="ppb-btn" onclick="showChangePasswordForm()">Update password</button></span>';
}

document.addEventListener("DOMContentLoaded", function () {
  // Main dashboard only: modulesGrid. Module dashboards (HR, Inspection, etc.) use runNavVisibility for consistent nav
  const hasModuleGrid =
    !!document.getElementById("hrFormsGrid") ||
    !!document.getElementById("inspectionFormsGrid");
  const isDashboardPage =
    !!document.getElementById("modulesGrid") && !hasModuleGrid;
  const isReviewHistoryPage =
    document.body.classList.contains("review-dashboard");
  const hasMainNav =
    document.getElementById("nav") &&
    document.querySelector("#nav .nav-center");

  // Run nav visibility on any page with main dashboard nav (admin, hr, mmr, store, etc.)
  ...
  if (isDashboardPage) {
    ...
    loadUserWelcome();
    document
      .querySelectorAll(".dashboard-stat-card--clickable")
      .forEach(bindDashboardStatCard);
    loadDashboardStats();
    ...
    if (userStr) {
      try {
        ...
        if (token) {
          fetch("/api/auth/me", {
            ...
            .then(function (data) {
              if (data && data.user) {
                ...
                checkAndShowAdminMenu(data.user);
                updateModuleVisibility(data.user);
                applyPasswordPolicyBanner(data.user, data);
                if (typeof loadPendingCount === "function") {
                  loadPendingCount(data.user);
                }
              }
          ...
          applyPasswordPolicyBanner(user);
        }
      } catch (e) {
        console.error("Error parsing user from localStorage:", e);
      }
  ...
  document.addEventListener("keydown", function (e) {
    if (
      e.key === "Escape" &&
      mobileMenuDrawer &&
      mobileMenuDrawer.classList.contains("active")
    ) {
      closeMobileMenu();
    }
  });
  ...
  if (submittedNavItem) {
    ...
    if (submittedTopLink) {
      submittedTopLink.addEventListener("click", function (e) {
        if (!submittedNavItem.classList.contains("has-submitted-dropdown"))
          return;
        // Desktop / fine pointer: rely on CSS :hover for the menu; allow normal navigation / modified clicks.
        const useClickToggle = window.matchMedia(
...
function initNotifications() {
  ...
  if (markAllReadBtn) {
    markAllReadBtn.addEventListener("click", async function () {
      try {
        const response = await authenticatedFetch(
          "/hr/api/notifications/mark-all-read",
          {
            method: "POST",
          },
        );
...
async function loadNotificationCount() {
  ...
  try {
    ...
    const response = await authenticatedFetch(
      "/hr/api/notifications/unread-count",
    );
    if (response.ok) {
      const data = await response.json();
  ...
  try {
    ...
    if (data.notifications && data.notifications.length > 0) {
      notificationList.innerHTML = data.notifications
        .map((n) => {
          const isInspection = (n.notification_type || "").startsWith(
            "inspection_",
          );
          const isPriority = n.notification_type === "cd_priority";
          ...
          return `
 <div class="notification-item ${n.is_read ? "" : "unread"} ${isPriority ? "priority" : ""}" onclick="markNotificationRead(${n.id}, '${(n.submission_id || "").replace(/'/g, "\\'")}', '${(n.notification_type || "").replace(/'/g, "\\'")}')">
 <div class="notification-icon ${iconClass}">${iconEmoji}</div>
 <div class="notification-content">
 <div class="notification-title">${isPriority ? '<span class="notification-priority-badge">Priority</span>' : ""}${escapeHtml(n.title)}</div>
...
async function markNotificationRead(id, submissionId, notificationType) {
  try {
    ...
    if (submissionId) {
      if (notificationType === "cd_priority") {
        window.location.href = "/inspection/notifications";
        return;
      }
      ...
      if (notificationType === "hr_mgmt_chain_signoff") {
        window.location.href =
          "/hr/mgmt-sign/" + encodeURIComponent(submissionId);
        return;
      }
      if (notificationType === "hr_replacement_signoff") {
        window.location.href =
          "/hr/replacement-sign/" + encodeURIComponent(submissionId);
        return;
      }
