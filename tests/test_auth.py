...
class TestRegister:
    """Test registration endpoint (opt-in; public signup is off by default)."""
...
class TestPublicRegistrationClosed:
    def test_register_api_disabled_by_default(self, client, app):
        with app.app_context():
            response = client.post('/api/auth/register', json={
                'first_name': 'New',
                'last_name': 'User',
    ...
    def test_register_page_redirects_to_login(self, client):
