...
Callable from app startup via seed_supervisors_teams() inside an app context.
"""
import os
...
def seed_supervisors_teams(password=None):
    """Idempotent demo supervisors + technicians. Requires app context."""
    password = password or os.environ.get('SEED_TEAM_PASSWORD', 'DemoTech2026!')
    ...
    return {
        ...
        'created_technicians': created_technicians,
    }
...
def main():
    ...
    with app.app_context():
        ...
        print(f'Shared demo password: {result["password"]}')
        if result['created_supervisors']:
            print(f'Created supervisor account(s): {", ".join(result["created_supervisors"])}')
        if result['created_technicians']:
            print(f'Created technician login(s): {", ".join(result["created_technicians"])}')
