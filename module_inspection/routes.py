# module_inspection/routes.py — unified inspection form
import os
import json
import time
import base64
...
def _has_inspection_access(user):
    """Admin or any legacy HVAC/Civil/Cleaning flag grants unified Inspection access."""
    if not user:
        return False
    if user.role == 'admin':
        return True
...
try:
    from .inspection_generators import create_excel_report, create_pdf_report
    logger.info("✅ Successfully imported inspection_generators")
...
try:
    from . import generator as inspection_generator
except Exception:
    inspection_generator = None  # we'll still allow fallback behavior if needed
...
def _normalize_legacy_form_data(form_data, submission=None):
    """Map legacy civil/cleaning shapes onto the unified HVAC-style form_data."""
    if not isinstance(form_data, dict):
        return {}
    fd = dict(form_data)
    if not fd.get('items') and isinstance(fd.get('work_items'), list):
        fd['items'] = fd['work_items']
    if not fd.get('site_name'):
        fd['site_name'] = (
            fd.get('project_name')
            or fd.get('location')
            or (getattr(submission, 'site_name', None) if submission else None)
            or ''
        )
    if not fd.get('visit_date'):
        fd['visit_date'] = (
            fd.get('date_of_visit')
            or fd.get('date')
            or (getattr(submission, 'visit_date', None) if submission else None)
            or ''
        )
    if submission is not None and not fd.get('category'):
        mt = (getattr(submission, 'module_type', None) or '').lower()
        fd['category'] = {
            'hvac_mep': 'HVAC',
            'hvac': 'HVAC',
            'civil': 'Civil',
            'cleaning': 'Cleaning',
            'inspection': fd.get('category') or 'General',
        }.get(mt, fd.get('category') or 'General')
    return fd
...
def inspection_dashboard():
    """Unified Inspection hub — one form entry point."""
...
@inspection_bp.route("/form", methods=["GET"])
@jwt_required()
def index():
    """Inspection form - requires authentication and module access. Supports editing existing submissions."""
    ...
    try:
        ...
        if edit_submission_id:
            ...
            legacy_ok = (submission.module_type or '') in (
                'inspection', 'hvac_mep', 'hvac', 'civil', 'cleaning'
            )
            if not legacy_ok:
                return render_template('access_denied.html',
                                     module='Inspection',
                                     message='This submission is not an inspection form.'), 404
...
@inspection_bp.route("/dropdowns", methods=["GET"])
@jwt_required()
def dropdowns():
...
@inspection_bp.route("/save-draft", methods=["POST"])
@jwt_required()
def save_draft():
    ...
    try:
        ...
        if visit_date:
            try:
                ...
                logger.info(f"Date validation (Inspection): parsed_date={parsed_date}, today_utc={today_utc}, max_allowed={max_allowed_date}")
...
@inspection_bp.route("/status/<job_id>", methods=["GET"])
@jwt_required()
def status(job_id):
    try:
...
@inspection_bp.route("/generated/<path:filename>", methods=["GET"])
@jwt_required()
...
@inspection_bp.route("/download/submission/<submission_id>/<file_type>", methods=["GET"])
@jwt_required()
def download_by_submission(submission_id, file_type):
    ...
    return redirect(url_for('inspection_bp.download_file', job_id=job_id, file_type=file_type))


@inspection_bp.route("/download/<job_id>/<file_type>", methods=["GET"])
@jwt_required()
...
@inspection_bp.route("/upload-photo", methods=["POST"])
@jwt_required()
...
@inspection_bp.route("/submit-with-urls", methods=["POST"])
@jwt_required()
@rate_limit_if_available('10 per minute')  # Limit form submissions
def submit_with_urls():
    ...
    try:
        ...
        return jsonify({
            ...
            "job_id": job_id,
            "job_status_url": url_for("inspection_bp.status", job_id=job_id, _external=False)
        })
...
@inspection_bp.route("/add-photos-to-item", methods=["POST"])
@jwt_required()
