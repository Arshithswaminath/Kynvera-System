"""Kynvera-branded sales quotation PDF."""
...
from common.kynvera_pdf_brand import (
    PRIMARY as BRAND,
    PRIMARY_DARK,
    TEXT_DARK as INK,
    TEXT_MUTED as MUTED,
    HAIRLINE as RULE,
    SURFACE_ALT as ROW_ALT,
    SOFT_WASH as HEADER_BG,
    resolve_logo_path,
    COMPANY_NAME,
)
...
STAMP_BOX = colors.HexColor('#fafafb')
...
def _stamp_cell():
    inner = Table(
        [
            [_p(COMPANY_NAME.upper(), 7, bold=True, color=MUTED, align=TA_CENTER)],
...
def _section(title, body):
    ...
    return [
        Spacer(1, 6),
        _p(title, 10, bold=True, color=PRIMARY_DARK),
        Spacer(1, 2),
...
def build_quotation_pdf(quotation, output_path: str) -> str:
    ...
    logo_path = resolve_logo_path(prefer_wordmark=True)
    logo_cell = Spacer(1, 1)
    if logo_path:
        try:
            logo_cell = RLImage(logo_path, width=42 * mm, height=14 * mm)
        except Exception:
            logo_cell = Spacer(1, 1)
    ...
    headers = [
        _p('<b>S.No.</b>', 8, bold=True, color=INK, align=TA_CENTER),
        _p('<b>Description</b>', 8, bold=True, color=INK),
        _p('<b>Qty.</b>', 8, bold=True, color=INK, align=TA_CENTER),
        _p('<b>U.Price</b>', 8, bold=True, color=INK, align=TA_RIGHT),
        _p('<b>Total</b>', 8, bold=True, color=INK, align=TA_RIGHT),
    ]
    rows = [headers]
