...
InspectionNotification / TicketProject). No ML: the "probability" is a simple
stage-based win likelihood applied to deal value (weighted forecast), with win
rates and the conversion funnel read straight from history.

The admin BD dashboard route fetches the data and merges these results into its
existing `stats` payload; the template renders them.
"""
...
# Default win probability per pipeline stage. Editable / auto-calibratable later
# (see plan "Notes / future"); for v1 these are sensible static priors.
STAGE_WIN_PROB = {
    'prospecting': 0.10,
    'qualifying': 0.25,
    'proposal': 0.50,
...
# Deals in these statuses are still live (not won/lost) and count toward forecast.
_OPEN_STATUSES = {'active', 'prospect', 'proposal'}

# A live deal with no movement for this many days is "stalled".
STALL_DAYS = 30
...
def stalled_deals(projects, limit=12):
    ...
    # Most idle / highest value first.
    out.sort(key=lambda d: (-(d['idle_days'] or 0), -d['value']))
    return {'count': len(out), 'rows': out[:limit]}


# ---------------------------------------------------------------------------
# Outcome loop — Civil Defense compliance + BD → delivery conversion
# ---------------------------------------------------------------------------

def outcome_loop(notifications, ticket_projects):
    """Civil Defense pass/fail + reinspection rates, and BD-win → ticketing realised value."""
    today = date.today()
    total = len(notifications)
    passed = sum(1 for n in notifications if (n.outcome or '') == 'pass')
    failed = sum(1 for n in notifications if (n.outcome or '') == 'fail')
    decided = passed + failed
    reinspections = sum(int(n.reinspection_count or 0) for n in notifications)
    overdue = sum(
        1 for n in notifications
        if n.status not in ('completed', 'closed')
        and n.inspection_date and n.inspection_date < today
    )
    actionable = sum(1 for n in notifications if getattr(n, 'can_convert', False))
    ...
    return {
        ...
        'cd_pass': passed,
        'cd_fail': failed,
        'cd_pass_rate': int(round(100 * passed / decided)) if decided else 0,
        'cd_reinspection_rate': round(reinspections / total, 2) if total else 0,
        'cd_overdue': overdue,
        'cd_actionable': actionable,
