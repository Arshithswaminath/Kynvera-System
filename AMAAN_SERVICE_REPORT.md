...
| **SERVICE REPORT** title | Fixed in PDF (bold, underlined) | |
| Tel / Fax / P.O. Box / email / website | Fixed Amaan contact block (`CONTACT_LINES` in PDF builder) | Tel +971 6 741 2778 · Fax +971 6 741 2595 · P.O. Box 7777, Ajman, UAE · info@amaanajh.com · www.amaanajh.com |
...
The UI in `module_ticketing/templates/ticket_service_report.html` adopts the **inspection-suite "new skin"** (same visual language as the Civil / Cleaning / HVAC forms in `inspection-forms-unified.css`): a rounded container card with a **red gradient header** (`#3d0509 → #d21725`), red section sub-headings with a left accent bar, brand-red fields/toggles/segments, and a sticky Save / PDF / Print bar. The styles are self-contained in the template `<style>` block (the shared inspection CSS is Bootstrap/stepper-coupled). It is organized as:
...
The builder reproduces the paper template (`Service Report …_ITEM No. 1.pdf`) faithfully.
Built in order by `build_service_report_pdf()`:

1. **Header** — Logo \| **SERVICE REPORT** (underlined) \| red contact block + SRN (open, no box)  
2. **General info grid** — single boxed table mirroring the paper: Customer (tall) / Job No / Page; Site Name / Location; Engineer-Technician; **Fire Alarm System** (Qty/Type/Make/No. of Zones-Loops — these live in this grid, not the systems band); a tall **Date** cell; and the **Time** table (Arrive/Left/Travel/Total × Hrs/Mns) spanning the right edge  
3. **Fire Fighting + Type of Job band** — left: 2-up Fire Fighting checklist, each with its own **vector checkbox**; right: Type of Job single-choice **vector radios** (`_checkbox()` / `_radio()` are drawn shapes — font-independent, so checked/unchecked render on any platform incl. Linux servers)  
4. **Comments** — header row + ruled lines (min 10)  
5. **Parts** — side-by-side Parts Used \| Parts Required tables (padded to 6 rows)  
6. **Customer remarks** — header row + 3 ruled lines  
7. **Signatures** — Customer (Name / Signature / Mobile No) \| Engineer-Technician (Name / Signature / ID No) with embedded signature images  
...
*Last updated: June 2026 — paper-faithful PDF rewrite (intricate top grid, vector checkboxes/radios, authoritative contact block) + web form re-skinned to the inspection-suite design language.*
