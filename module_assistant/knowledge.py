...
_GENERIC_BRAND_TOKENS = {'injaaz', 'application', 'platform', 'app', 'fm', 'injaazfm'}
...
def _is_definition_faq(question: str, faq_id: str = '') -> bool:
    q = (question or '').lower()
    return faq_id == 'what_is_injaaz' or q.startswith('what is ') or q.startswith("what's ")
...
def search_faqs(query: str, limit: int = 2) -> List[dict]:
    ...
    for faq in _all_knowledge():
        ...
        # Reward records that cover the user's specific intent (e.g. "locations"), not just "injaaz".
        for tok in specific:
