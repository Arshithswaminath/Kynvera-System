"""
Keyword-based knowledge search for the Amaan assistant (no embeddings).
...
_GENERIC_BRAND_TOKENS = {'amaan', 'application', 'platform', 'app', 'fm', 'amaanfm'}
...
def search_faqs(query: str, limit: int = 2) -> List[dict]:
    ...
    for faq in _all_knowledge():
        ...
        # Penalise records that miss every specific query token (e.g. generic "What is Amaan?").
        elif specific and not any(_token_hits_record(tok, question, answer, keywords, tags) for tok in specific):
            score -= 12.0

        # "Where is Amaan?" must not return a "What is Amaan Application?" definition card.
    ...
    for sc, faq in scored[:limit]:
        ...
        results.append({
            ...
            'answer': answer,
            'source': faq.get('source', 'Amaan Help'),
            'link': faq.get('link'),
